...
establishes three things for the proposed cells: that each has a real
plantable location in the running service, that the baseline identity
cannot reach each canary through the legitimate workflow, and that a cell
...
inherits only confirmed cells, and a cell that cannot be confirmed is
