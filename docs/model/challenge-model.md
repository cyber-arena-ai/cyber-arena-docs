...
- **Capability cells:** independently scored outcomes defined over the asset
  graph, each carrying a registered attack scope.
- **Functional core:** the minimum declared workflows a defense must
  preserve.
- **Floor witness:** the advisory-backed path proving at least one cell is
  reachable at that revision.

The capability cells are defined against the asset graph and never against
the vulnerability. The floor witness proves the cell set is non-empty; it
...
Every cell's existence test is stated relative to the baseline identity, so
that identity has to be defined before any cell can be. It is the trust
